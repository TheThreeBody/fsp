...
$(function() {
    webside.form.channel.validateChannelForm();
});
</script>
<div class="page-header">
	<h1>
		<c:if test="${empty channelEntity}">
		新增角色
		</c:if>
		<c:if test="${!empty channelEntity}">
		编辑角色
...
<div class="row" style="margin-top:5px;">
	<div class="col-xs-12">
		...
		<c:if test="${!empty channelEntity}">
			...
			<input type="hidden" name="id" id="roleId" value="${channelEntity.id }">
		</c:if>
		   <div class="form-group">
		      <label class="control-label col-sm-1 no-padding-right" for="name">渠道名称</label>
		      <div class="col-sm-10">
		      <div class="clearfix">
		         <input class="form-control" name="name" id="name" type="text" 
		           value="${channelEntity.name }" placeholder="角色名称..."/>
		      </div>
		      </div>
		   </div>
			<div class="form-group">
