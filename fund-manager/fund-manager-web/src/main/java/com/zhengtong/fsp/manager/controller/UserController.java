...
import com.zhengtong.fsp.commons.utils.PageUtil;
import com.zhengtong.fsp.commons.utils.RandomUtil;
import com.zhengtong.fsp.manager.support.shiro.util.ShiroEndecryptTools;
...
import com.zhengtong.fsp.manager.pojo.UserInfoEntity;
import com.zhengtong.fsp.manager.service.RoleService;
import com.zhengtong.fsp.manager.service.UserService;
...
public class UserController extends BaseController {
	...
	public String listUI(Model model, HttpServletRequest request) {
		try
		{
			...
			return SysConstant.BACKGROUND_PATH + "/user/list";
		}catch(Exception e)
		{
	...
	public Object list(String gridPager, HttpServletResponse response) throws Exception{
		...
		boolean isAdmin = ShiroAuthenticationManager.getSubject().hasRole("administrator");
		if(!isAdmin) {
...
//				ExportUtils.exportExcel(response,pager,pager.getExportDatas(),"");
	...
	public String addUI(Model model) {
		try
		{
			...
			return SysConstant.BACKGROUND_PATH + "/user/form";
		}catch(Exception e)
		{
			throw new AjaxException(e);
		}
	...
	public Object add(UserEntity userEntity) throws AjaxException
	{
		...
		try
		{
			...
			UserEntity user = ShiroEndecryptTools.md5Password(userEntity.getAccountName(), userEntity.getPassword(), 2);
			//设置添加用户的密码和加密盐
	...
	public Object resetPassword(UserEntity userEntity){
		...
		try
		{
			...
			//加密用户输入的密码，得到密码和加密盐，保存到数据库
			UserEntity user = ShiroEndecryptTools.md5Password(userEntity.getAccountName(), password, 2);
			//设置添加用户的密码和加密盐
			userEntity.setPassword(user.getPassword());
			userEntity.setCredentialsSalt(user.getCredentialsSalt());
	...
	public String infoUI(Model model, Long id) {
		try
		{
			UserEntity userEntity = userService.findById(id);
			model.addAttribute("userEntity", userEntity);
			return SysConstant.BACKGROUND_PATH + "/user/info";
		}catch(Exception e)
		{
			throw new AjaxException(e);
		}
	...
	public String passwordUI(Model model, UserEntity userEntity) {
		...
		{
			model.addAttribute("userEntity", userEntity);
			return SysConstant.BACKGROUND_PATH + "/user/password";
		}catch(Exception e)
		{
			throw new AjaxException(e);
		}
	...
	public Object password(UserEntity userEntity){
		...
		try
		{
			...
			//加密用户输入的密码，得到密码和加密盐，保存到数据库
			UserEntity user = ShiroEndecryptTools.md5Password(userEntity.getAccountName(), userEntity.getPassword(), Integer.valueOf(hashIterations));
			//设置添加用户的密码和加密盐
			userEntity.setPassword(user.getPassword());
			userEntity.setCredentialsSalt(user.getCredentialsSalt());
			int cnt = userService.updatePassword(userEntity, password);
